"""
Loss-matched geometry control. For each (arch, seed):

  1. Read Adam's final training loss L_A.
  2. Find the SGD epoch t* with closest training loss.
  3. Load SGD's epoch-t* checkpoint and Adam's final checkpoint.
  4. Compare lambda_max(H), tr(H), parameter distance from init, and
     function-space disagreement at matched loss.

Output: results_part2/loss_matched_geometry.json
"""
import os
import json
import time
...
def parse_args():
    ...
    p.add_argument("--out", default=os.path.join(RESULTS_DIR, "loss_matched_geometry.json"))
    ...
    for arch in ARCHS:
        for seed in SEEDS:
            ...
            if "train_loss_per_epoch" not in sgd_d:
                print(f"SKIP {arch}/seed={seed}: missing train_loss_per_epoch field", flush=True)
                continue

            adam_final_loss = adam_d["train_loss_per_epoch"][-1]
            e_star, matched_loss = find_loss_matched_epoch(
                sgd_d["train_loss_per_epoch"], adam_final_loss
            )
            print(f"  {arch}/seed={seed}: Adam loss={adam_final_loss:.4f}  "
                  f"SGD t*={e_star} (loss={matched_loss:.4f})", flush=True)
            ...
            print(f"    SGD@t*: lam_max={sharp_sgd_t:.2f} tr={trace_sgd_t:.2f} d={d_sgd_t:.2f}    "
                  f"Adam: lam_max={sharp_ada:.2f} tr={trace_ada:.2f} d={d_ada:.2f}", flush=True)

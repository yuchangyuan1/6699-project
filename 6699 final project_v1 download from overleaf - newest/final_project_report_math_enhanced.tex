...
\title{\textbf{Do Adaptive Optimizers Alter the Implicit Bias and Generalization of Neural Networks?}\\
\Large A Matched-Seed Comparison of SGD and Adam}
...
\begin{abstract}
Adaptive optimizers such as Adam are often preferred in practice because they reduce training loss quickly and require less manual learning-rate tuning than stochastic gradient descent (SGD). For this project, the more interesting question is whether this advantage is only about speed, or whether adaptivity also changes the implicit bias of training and the generalization behavior of the learned network. We study this question through a controlled comparison of SGD with momentum and Adam on Fashion-MNIST, using two architectures: a batch-normalized multilayer perceptron (MLP) and a small convolutional neural network (SmallCNN). Across five matched random seeds, both optimizers start from identical initial weights and see the same minibatch ordering. We compare early-stage convergence, solution geometry, and final generalization.

The results suggest that the answer is architecture-dependent. Adam reaches loss thresholds faster than the default SGD baseline, with the clearest advantage in the MLP. Adam also moves much farther from initialization and produces more dispersed final parameters across seeds, so the two optimizers are not simply taking the same path at different speeds. In the MLP, Adam converges to substantially flatter Hessian-proxy solutions ($5\times$ lower dominant Hessian eigenvalue, $6\times$ lower Hessian trace), yet achieves test accuracy and train--test gap comparable to SGD's. This shows that flatter geometry does not automatically give a generalization advantage. In the SmallCNN, Adam's dominant eigenvalue is higher than SGD's while the Hessian trace is comparable, and the generalization difference is small in this setting. Overall, adaptive optimization changes the training trajectory and solution geometry, but its effect on generalization depends on the architecture and on which geometry metric is considered.
\end{abstract}
...
\end{quote}
We divide this broad question into three parts:
\begin{enumerate}
